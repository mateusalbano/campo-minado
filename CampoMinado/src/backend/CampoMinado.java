...
public class CampoMinado {
	...
	private boolean casaValida(int i, int j) {
	...
	private void verificaVizinhos(Queue<int[]> filaCasas, boolean[][] casaVisitada,int posI, int posJ) {
		
		for (int i = posI - 1; i <= posI + 1; i++) {
			for (int j = posJ - 1; j <= posJ + 1; j++) {

				if (casaValida(i, j) && campo[i][j] != -1 && !(i == posI && j == posJ) && !casaVisitada[i][j]) {
					filaCasas.add(new int[] {i, j});
					casaVisitada[i][j] = true;
				}
	...
	private void abreCasas(int posI, int posJ) {
		ConcurrentLinkedQueue<int[]> filaCasas = new ConcurrentLinkedQueue<>();
		boolean[][] casaVisitada = new boolean[n1][n2];
		int[] casa = new int[] {posI, posJ};
		casaVisitada[posI][posJ] = true;
		filaCasas.add(casa);
		while (casa != null) {
			int i = casa[0];
			int j = casa[1];
			if (i >= 0 && i < n1 && j >= 0 && j < n2 && campo[i][j] != -1) {
				if (campo[i][j] == 0) {		
					verificaVizinhos(filaCasas, casaVisitada, i, j);
				}
				casaAberta[i][j] = true;
			}
	...
	private void checaBomba(int posI, int posJ) {
		...
		for (int i = posI - 1; i <= posI + 1; i++) {
			for (int j = posJ - 1; j <= posJ + 1; j++) {
				if(casaValida(i, j) && casaTemBomba(i, j) && !(i == posI && j == posJ)) {
					campo[posI][posJ]++;
				}
			}
		}
